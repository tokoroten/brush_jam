...
describe('planMask', () => {
  it('produces crop-local, dilated shapes', () => {
    const plan = planMask([{ x: 1500, y: 1500, width: 20, height: 20 }], crop, 768, 48, 32);
  ...
  it('reports empty when nothing intersects the crop', () => {
    expect(planMask([{ x: 3000, y: 3000, width: 4, height: 4 }], crop, 768).empty).toBe(true);
  });

  it('keeps every shape inside the crop bounds', () => {
    const plan = planMask([{ x: 1000, y: 1000, width: 1024, height: 1024 }], crop, 768);

...
Codex 第 9 ラウンドの判定は「条件付き Go」。条件は「ワーカーを事前に warm しておく」「セッション中にワーカーの設定変更・再起動をしない」「`AI_BACKEND=stream` を .env で固定する(実施済み)」。残件 3 件(能力再取得時の恒久エラー再試行ループ、5xx 判定の正規表現が `512` に誤反応、auto 選択時のプロファイル降格)は朝までに修正予定。
